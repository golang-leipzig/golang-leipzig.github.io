...
var glyphs = map[byte][glyphH]uint8{
	...
	'\xb7': {0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00}, // middot
}
...
func drawGIFScene(cs []Customer, tri *[3]Point, best *Point, trail []Point, iter int, val float64) *image.Paletted {
	var (
		img = newGIFFrame()
	...
	drawText(img, padX, *flagHeight-padY+8, 2, fmt.Sprintf("%d \xb7 %.8f", iter, val), dim)
...
func writeGIF(path string, cs []Customer, hist [][3]Point, opt Point) error {
	...
	for i := 0; i < len(hist); i += step {
		...
		frame := drawGIFScene(cs, &tri, &best, trail, i, val)
	...
	final := drawGIFScene(cs, &lastTri, &opt, trail, len(hist)-1, objective(opt, cs))
